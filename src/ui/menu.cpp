...
public:
  ...
  // more kOpenMenuItemMessage or kCloseMenuItemMessage messages are in the
  // queue.
...
void Menu::showPopup(const gfx::Point& pos)
{
  ...
  // Menubox position
  window->positionWindow(MID(0, pos.x, ui::display_w() - window->bounds().w),
  ...
  default:
    if (msg->type() == kOpenMenuItemMessage)
    {
      ...
      else
      {
        ...
        if (s2.isEmpty())
          x = x_right; // Use the right because there aren't intersection with
                       // it
        else if (s1.isEmpty())
...
void MenuItem::startTimer()
{
  if (m_submenu_timer == nullptr)
    m_submenu_timer = std::make_unique<Timer>(kTimeoutToOpenSubmenu, this);
