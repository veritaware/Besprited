...
struct GetPixelsDelegateRgba
{
  ...
  GetPixelsDelegateRgba(std::vector<std::vector<uint8_t>>& channel)
    : channel(channel)
  {
  }
...
struct GetPixelsDelegateGrayscale
{
  ...
  GetPixelsDelegateGrayscale(std::vector<std::vector<uint8_t>>& channel)
    : channel(channel)
  {
  }
...
struct GetPixelsDelegateIndexed
{
  ...
  void operator()(IndexedTraits::pixel_t color)
  {
    ...
    {
      const color_t rgb = pal->getEntry(color);
...
void MedianFilter::applyToIndexed(FilterManager* filterMgr)
{
  ...
  for (; x < x2; ++x)
  {
    ...
    else
    {
      ...
      color = static_cast<int>(pal->getEntry(color));
