...
TaskMonitor - A helper class for automatically canceling a task when the monitor
is destroyed
...
TaskManager::instance().addTask<Data>(
  [](std::atomic_bool& isAlive) -> Data {
    ...
    isAlive = false;
    return Data{}; // each time it is executed, a Data will be passed to the
consumer below
  },
  [](Data data) {
...
public:
  TaskHandle(std::shared_ptr<detail::Task> task = nullptr)
    : task{task}
  {
  }
...
public:
  void operator=(const TaskHandle& handle) { m_handle = handle; }

  operator bool() { return m_handle.done(); }

  ~TaskMonitor() { m_handle.abort(); }
};
