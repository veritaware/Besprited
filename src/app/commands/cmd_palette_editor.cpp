...
private:
  ...
  app::Color::Type m_type = app::Color::MaskType;
  Box m_vbox;
  Box m_topBox;
  Box m_bottomBox;
...
static PaletteEntryEditor* g_window = nullptr;
...
PaletteEditorCommand::PaletteEditorCommand()
  : Command("PaletteEditor", "Palette Editor", CmdRecordableFlag)
{
  m_open = true;
  m_close = false;
  m_switch = false;
...
void PaletteEditorCommand::onExecute(Context* context)
{
  ...
  // If the frame is already created and it's visible, close it (only in
  // "switch" or "close" modes)
