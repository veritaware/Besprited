...
app::Color ColorWheel::getColorInClientPos(const gfx::Point& pos)
{
  ...
  if (d < m_wheelRadius + 2 * guiscale())
  {
    ...
    return app::Color::fromHsv(MID(0, hue, 360), MID(0, sat, 100), 100);
  }
...
void ColorWheel::onPaint(ui::PaintEvent& ev)
{
  ...
  for (int y = rc.y; y < rc.y + rc.h; ++y)
  {
    ...
    {
      app::Color appColor = ColorWheel::getColorInClientPos(gfx::Point(x, y));
...
void ColorWheel::onOptions()
{
  ...
  case Harmony::COMPLEMENTARY:
    complementary.setSelected(true);
    break;
  case Harmony::MONOCHROMATIC:
    monochromatic.setSelected(true);
    break;
  case Harmony::ANALOGOUS:
    analogous.setSelected(true);
    break;
  case Harmony::SPLIT:
    split.setSelected(true);
    break;
  case Harmony::TRIADIC:
    triadic.setSelected(true);
    break;
  case Harmony::TETRADIC:
    tetradic.setSelected(true);
    break;
  case Harmony::SQUARE:
