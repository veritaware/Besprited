...
void SkinTheme::loadFonts(const std::string& skinId)
{
  ...
  // ToDo: add the ability to set preferred font size in skin XML + application
  // preferences
  {
    FontList fonts{};
  ...
  // Load styles
  {
    ...
    while (xmlStyle)
    {
      ...
      while (xmlRule)
      {
        ...
        {
          if (align)
            (*style)[StyleSheet::iconAlignRule()] = css::Value(align);
          if (part_id)
            (*style)[StyleSheet::iconPartRule()] = css::Value(part_id);
        ...
        {
          if (color_id)
            (*style)[StyleSheet::textColorRule()] = css::Value(color_id);
          if (align)
            (*style)[StyleSheet::textAlignRule()] = css::Value(align);
...
#define BORDER4(L, T, R, B) widget->setBorder(gfx::Border((L), (T), (R), (B)))
  ...
  case kSeparatorWidget:
    // Frame
    if (widget->align() & HORIZONTAL && widget->align() & VERTICAL)
    {
      BORDER(4 * scale);
    }
    // Horizontal bar
...
void SkinTheme::paintBox(PaintEvent& ev)
{
  ...
  if (!widget->isTransparent() && !is_transparent(BGCOLOR))
  {
...
void SkinTheme::paintButton(PaintEvent& ev)
{
                        ...
                        const SkinPartPtr& normal) -> SkinPartPtr
  {
    if (look == MiniLook)
      return mini;
    if (look == LeftButtonLook)
      return left;
    if (look == RightButtonLook)
      return right;
    return normal;
  };
  ...
  {
    if (widget->isSelected())
      icon.offset(get_button_selected_offset(), get_button_selected_offset());
...
void SkinTheme::paintEntry(PaintEvent& ev)
{
  ...
  if (c == caret && state && widget->hasFocus() && widget->isEnabled())
  {
    ...
    if (window->hasText())
    {
      ...
      styles.windowTitle()->paint(g,
                                  Rect(cpos.x, pos.y + 3 * guiscale(),
                                       cpos.w, // TODO this hard-coded 3 should
                                               // be configurable in skin.xml
                                       window->textHeight()),
...
void SkinTheme::paintTooltip(PaintEvent& ev)
{
  ...
  {
  case TOP | LEFT:
    nw = parts.tooltipArrow()->bitmapNW();
    break;
  case TOP | RIGHT:
    ne = parts.tooltipArrow()->bitmapNE();
    break;
  case BOTTOM | LEFT:
    sw = parts.tooltipArrow()->bitmapSW();
    break;
  case BOTTOM | RIGHT:
    se = parts.tooltipArrow()->bitmapSE();
    break;
  default:
    break;
  }
...
void SkinTheme::drawHline(Graphics* g, const Rect& rc, const SkinPart* skinPart)
{
  ...
  for (x = rc.x; x < rc.x2() - skinPart->size().w; x += skinPart->size().w)
  {
...
void SkinTheme::drawVline(Graphics* g, const Rect& rc, const SkinPart* skinPart)
{
  ...
  for (y = rc.y; y < rc.y2() - skinPart->size().h; y += skinPart->size().h)
  {
    ...
    if (base::get_file_extension(themeFont) != "png")
    {
      ...
      for (auto& dir : base::get_font_paths())
      {
        ...
        for (const auto child : item->children())
        {
          if (child->isFolder())
            continue;
          if (auto shortName = base::string_to_lower(child->displayName());
              shortName.find(themeLower) != std::string::npos)
