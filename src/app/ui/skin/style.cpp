...
void Rule::paint(ui::Graphics* g, const gfx::Rect& bounds, const char* text)
{
  ...
  , m_icon(nullptr)
{
  ...
  if (iconAlign != none || iconPart != none || iconX != none || iconY != none)
  {
    m_icon = new IconRule();
...
void Rules::paint(ui::Graphics* g, const gfx::Rect& bounds, const char* text)
{
  if (m_background)
    m_background->paint(g, bounds, text);
  if (m_icon)
    m_icon->paint(g, bounds, text);
  if (m_text)
    m_text->paint(g, bounds, text);
}
...
gfx::Size Rules::sizeHint(const char* text, int maxWidth)
{
  ...
  if (m_text && text)
  {
    ...
    if (sz.w > 0)
      sz.w += 2 * ui::guiscale(); // TODO text separation
    sz.w += textSize.w;
...
gfx::Size Style::sizeHint(const char* text, const State& state, int maxWidth)
{
