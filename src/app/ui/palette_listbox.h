...
/*
PaletteListBoxes are variants of ListBoxes that are only meant to show two kinds
of ListItem:
...
Since it is just a ui widget, it doesn't do much on its own and must be
populated. PaletteFileListBox is a variation that populates itself by looking
for palettes in the Filesystem.
*/
...
public:
  ...
  {
    setLoading(true);
    m_resourcesLoader->load(
        [this](Resource palResource)
        {
          if (palResource)
          {
