...
{
public:
  explicit WidgetObject(ui::Widget* widget)
    : m_widget(widget)
  {
  }
  ...
  template <typename T> T* as() const { return dynamic_cast<T*>(m_widget); }
...
public:
  EntryObject()
    : WidgetObject(new CustomEntry(40, app::AppScripting::getFileName()))
  {
  }
...
public:
  CustomIntEntry(const std::string& fileName)
    : ui::IntEntry(0, 100)
    , m_fileName(fileName)
  {
  }
...
public:
  IntEntryObject()
    : WidgetObject(new CustomIntEntry(app::AppScripting::getFileName()))
  {
  }
...
public:
  CustomPaletteListBox(const std::string& fileName)
    : m_fileName(fileName)
  {
  }
...
{
public:
  PaletteListBoxObject()
    : WidgetObject(new CustomPaletteListBox(app::AppScripting::getFileName()))
  {
  }
...
public:
  DialogObject()
    : m_dialog(std::make_shared<ui::Dialog>())
  {
  ...
  template <typename T> std::shared_ptr<T> addWidget(const std::string& id)
  {
    if (!m_dialog)
      return nullptr;
