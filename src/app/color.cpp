  ...
  if (str != "mask")
  {
        ...
        str.find("gray{") == 0)
    {
      ...
      while ((j = str.find_first_of(",}", i)) != std::string::npos)
      {
        ...
        if (c < 4)
          table[c++] = std::strtod(element.c_str(), nullptr);
        if (c >= 4)
          break;
        i = j + 1;
      }
    ...
    {
      color = Color::fromIndex(std::strtol(str.c_str() + 6, nullptr, 10));
    }
  }
...
double Color::getHue() const
{
  ...
  case Color::RgbType:
    return Hsv(Rgb(m_value.rgb.r, m_value.rgb.g, m_value.rgb.b)).hue();
...
double Color::getValue() const
{
  ...
  case Color::IndexType:
  {
    ...
    {
      uint32_t c = get_current_palette()->getEntry(i);
      return Hsv(Rgb(rgba_getr(c), rgba_getg(c), rgba_getb(c))).value() * 100.0;
    }
    else
      return 0.0;
  }
