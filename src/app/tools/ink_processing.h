...
template <typename Derived> class InkProcessing
{
public:
...
template <> void TransparentInkProcessing<RgbTraits>::processPixel(int x, int y)
{
...
template <> void MergeInkProcessing<RgbTraits>::processPixel(int x, int y)
{
  *m_dstAddress = rgba_blender_merge(*m_srcAddress, m_color, m_opacity);
}

template <> void MergeInkProcessing<GrayscaleTraits>::processPixel(int x, int y)
{
...
private:
  struct GetPixelsDelegate
  {
    ...
    GetPixelsDelegate(const Palette* pal, color_t maskColor)
      : pal(pal)
      , maskColor(maskColor)
    {
    }
...
template <> void ReplaceInkProcessing<RgbTraits>::processPixel(int x, int y)
{
...
template <> void JumbleInkProcessing<RgbTraits>::processPixel(int x, int y)
{
...
template <> void JumbleInkProcessing<IndexedTraits>::processPixel(int x, int y)
{
  pickColorFromArea(x, y);
...
template <> void XorInkProcessing<RgbTraits>::processPixel(int x, int y)
{
  *m_dstAddress = rgba_blender_neg_bw(*m_srcAddress, m_color, 255);
}

template <> void XorInkProcessing<GrayscaleTraits>::processPixel(int x, int y)
{
...
template <> void BrushInkProcessing<RgbTraits>::processPixel(int x, int y)
{
  alignPixelPoint(x, y);
...
template <> void BrushInkProcessing<GrayscaleTraits>::processPixel(int x, int y)
{
  alignPixelPoint(x, y);
...
template <> void BrushInkProcessing<IndexedTraits>::processPixel(int x, int y)
{
  alignPixelPoint(x, y);
