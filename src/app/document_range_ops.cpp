                                   ...
                                   const DocumentRange& to)
{
  ...
  {
  case Move:
    undoLabel = "Move Range";
    break;
  case Copy:
    undoLabel = "Copy Range";
    break;
  default:
    ASSERT(false);
...

  {
    ...
    case DocumentRange::kCels:
    {
           ...
           srcLayerIdx != srcLayerEnd;)
      {
             ...
             srcFrame != srcFrameEnd;)
        {
          ...
          {
          case Move:
            api.moveCel(srcLayer, srcFrame, dstLayer, dstFrame);
            break;
          case Copy:
    ...
    case DocumentRange::kFrames:
    {
           ...
           srcFrame != srcFrameEnd;)
      {
        ...
        {
        case Move:
          api.moveFrame(sprite, srcFrame, dstFrame);
          break;
        case Copy:
          api.copyFrame(sprite, srcFrame, dstFrame);
          break;
        }
        srcFrame += srcFrameStep;
        dstFrame += dstFrameStep;
    ...
    case DocumentRange::kLayers:
    {
      ...
      case Move:
        if (place == kDocumentRangeBefore)
        {
          for (LayerIndex i = from.layerBegin(); i <= from.layerEnd(); ++i)
          {
            api.restackLayerBefore(layers[i], layers[to.layerBegin()]);
          }
        }
        else if (place == kDocumentRangeAfter)
        {
          ...
          {
            api.restackLayerAfter(layers[i], layers[to.layerEnd()]);
          }
        }
        break;
      ...
      case Copy:
        if (place == kDocumentRangeBefore)
        {
          for (LayerIndex i = from.layerBegin(); i <= from.layerEnd(); ++i)
          {
            api.duplicateLayerBefore(layers[i], layers[to.layerBegin()]);
          }
        }
        else if (place == kDocumentRangeAfter)
        {
          for (LayerIndex i = from.layerEnd(); i >= from.layerBegin(); --i)
          {
            api.duplicateLayerAfter(layers[i], layers[to.layerEnd()]);
          }
        }
        break;
      }
