...
template <typename Func>
void algo_spline(double x0, double y0, double x1, double y1, double x2,
                 double y2, double x3, double y3, Func&& func)
{
  algo_spline(x0, y0, x1, y1, x2, y2, x3, y3, &func,
              [](int x, int y, int x2, int y2, void* ptr)
              { (*reinterpret_cast<Func*>(ptr))(x, y, x2, y2); });
}

double algo_spline_get_y(double x0, double y0, double x1, double y1, double x2,
                         double y2, double x3, double y3, double x);
