                                         ...
                                         const color_t bgColor)
{
  ...
  const color_t maskColor = image->maskColor();
  color_t srcMainColor, srcBgColor;
...
void Brush::setImageColor(ImageColor imageColor, color_t color)
{
  ...
  case ImageColor::MainColor:
    m_mainColor = std::make_unique<color_t>(color);
    break;
  case ImageColor::BackgroundColor:
    m_bgColor = std::make_unique<color_t>(color);
    break;
  }
  ...
  else
  {
    ...
    case kSquareBrushType:
      ...
      else
      {
        ...
        const int sa = std::lround(r * sin(m_angle * (PI / 180.0f)));
        const int ca = std::lround(r * cos(m_angle * (PI / 180.0f)));
        const int x1 = -ca - -sa;
        const int y1 = -sa + -ca;
        const int x2 = ca - -sa;
        const int y2 = sa + -ca;
    ...
    case kLineBrushType:
    {
      ...
      const int sa = std::lround(r * sin(m_angle * (PI / 180.0)));
      const int ca = std::lround(r * cos(m_angle * (PI / 180.0)));
      const int x1 = -ca + r;
      const int y1 = -sa + r;
      const int x2 = ca + r;
      const int y2 = sa + r;
