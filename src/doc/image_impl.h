...
template <class Traits> class ImageImpl : public Image
{
private:
  ...
  inline address_t getBitsAddress() { return m_bits; }

  inline const_address_t getBitsAddress() const { return m_bits; }
...
template <> inline void ImageImpl<IndexedTraits>::clear(color_t color)
{
  std::fill(m_bits, m_bits + width() * height(), color);
}
