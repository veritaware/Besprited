...
public:
  ...
  ImageIterator(const Image* image, const gfx::Rect& bounds, int x, int y)
    : ImageIteratorT<ImageTraits, pointer, reference>(image, bounds, x, y)
  {
  }
};
...
public:
  // GCC 4.6 needs these re-definitions here.
  typedef const typename ImageTraits::pixel_t* pointer;
  typedef const typename ImageTraits::pixel_t& reference;
...
public:
  ...
  enum
  {
    pixels_per_byte = BitmapTraits::pixels_per_byte
  };
