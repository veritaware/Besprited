...
void Palette::setEntry(int i, color_t color)
{
  ...
  // m_colors (see issue #219). Real, always-on bounds check.
  if (i < 0 || i >= size())
    return;
...
{
  int c, diff = 0;
  const int min = MIN(this->m_colors.size(), other.m_colors.size());
  const int max = MAX(this->m_colors.size(), other.m_colors.size());
...
int Palette::findBestfit(int r, int g, int b, int a, int mask_index) const
{
  ...
  const int size = MIN(256, m_colors.size());

  for (int i = 0; i < size; ++i)
  {
    const color_t rgb = m_colors[i];
