...
struct PalEntryWithIndexPredicate
{
  ...
  bool operator()(const PalEntryWithIndex& a, const PalEntryWithIndex& b)
  {
    ...
    {
      const Hsv hsv1(Rgb(rgba_getr(c1), rgba_getg(c1), rgba_getb(c1)));
      const Hsv hsv2(Rgb(rgba_getr(c2), rgba_getg(c2), rgba_getb(c2)));
    ...
    {
      value1 = (std::max({rgba_getr(c1), rgba_getg(c1), rgba_getb(c1)}) +
                std::min({rgba_getr(c1), rgba_getg(c1), rgba_getb(c1)})) /
               2;
      value2 = (std::max({rgba_getr(c2), rgba_getg(c2), rgba_getb(c2)}) +
                std::min({rgba_getr(c2), rgba_getg(c2), rgba_getb(c2)})) /
               2;
      break;
    }
...
Remap sort_palette(Palette* palette, SortPaletteBy channel, bool ascending)
{
  ...
  std::stable_sort(tmp.begin(), tmp.end(),
                   PalEntryWithIndexPredicate(channel, ascending));
