                            ...
                            int src_w, int src_h, BlendFunc blend)
{
  ...
  const fixed dx = fixdiv(itofix(src_w - 1), itofix(dst_w - 1));
  const fixed dy = fixdiv(itofix(src_h - 1), itofix(dst_h - 1));
  int old_x, new_x;
                          ...
                          fixed spr_dy, Delegate& delegate)
{
  ...
  const gfx::Rect maskBounds = (mask ? mask->bounds() : spr->bounds());

  for (int x = static_cast<int>(l_bmp_x); x <= static_cast<int>(r_bmp_x); ++x)
  {
...
template <class Traits> class GenericDelegate
{
public:
...
public:
  ...
  void putPixel(const Image* spr, int spr_x, int spr_y)
  {
    ...
    const color_t c = get_pixel_fast<IndexedTraits>(spr, spr_x, spr_y);
    if (c != m_mask_color)
      *m_it = c;
  }
...
public:
  void putPixel(const Image* spr, int spr_x, int spr_y)
  {
    ...
    const int c = get_pixel_fast<BitmapTraits>(spr, spr_x, spr_y);
    if (c != 0) // TODO
      *m_it = c;
  }
};
                                              ...
                                              fixed ys[4])
{
  ...
  cos_angle = cos(angle * (PI / static_cast<double>(0x800000)));
  sin_angle = sin(angle * (PI / static_cast<double>(0x800000)));

  if (cos_angle >= 0)
    fix_cos = static_cast<int>(cos_angle * 0x10000 + 0.5);
  else
    fix_cos = static_cast<int>(cos_angle * 0x10000 - 0.5);
  if (sin_angle >= 0)
    fix_sin = static_cast<int>(sin_angle * 0x10000 + 0.5);
  else
    fix_sin = static_cast<int>(sin_angle * 0x10000 - 0.5);
