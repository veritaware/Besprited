...
struct FLOODED_LINE
{                   // store segments which have been flooded
...
static inline bool color_equal_32(color_t c1, color_t c2, int tolerance)
{
  ...
  {
    const int r1 = rgba_getr(c1);
    const int g1 = rgba_getg(c1);
    const int b1 = rgba_getb(c1);
    const int a1 = rgba_geta(c1);
    const int r2 = rgba_getr(c2);
    const int g2 = rgba_getg(c2);
    const int b2 = rgba_getb(c2);
    const int a2 = rgba_geta(c2);
...
static inline bool color_equal_16(color_t c1, color_t c2, int tolerance)
{
  ...
  {
    const int k1 = graya_getv(c1);
    const int a1 = graya_geta(c1);
    const int k2 = graya_getv(c2);
    const int a2 = graya_geta(c2);
               ...
               void* data, AlgoHLine proc)
{
  ...
  const color_t src_color = get_pixel(image, x, y);
  ...
  auto* p = static_cast<FLOODED_LINE*>(&flood_buf[0]);
