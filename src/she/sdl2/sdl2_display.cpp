...
SDL2Display* unique_display = nullptr;
int display_scale;
  ...
  , m_restoredHeight(0)
{
  ...
  instance()->gfx(
      [&]
      {
        ...
        {
          std::cout << "Unsupported wm subsystem for tablets" << "\n";
        }
#endif
#elif __APPLE__
...
bool SDL2Display::setNativeMouseCursor(NativeCursor cursor)
{
  ...
  case she::kArrowCursor:
    applyCursor(SDL_SYSTEM_CURSOR_ARROW);
    return true;
  case she::kIBeamCursor:
    applyCursor(SDL_SYSTEM_CURSOR_IBEAM);
    return true;
  case she::kWaitCursor:
    applyCursor(SDL_SYSTEM_CURSOR_WAIT);
    return true;
  case she::kLinkCursor:
    applyCursor(SDL_SYSTEM_CURSOR_HAND);
    return true;
  case she::kForbiddenCursor:
    applyCursor(SDL_SYSTEM_CURSOR_NO);
    return true;
  case she::kMoveCursor:
    applyCursor(SDL_SYSTEM_CURSOR_SIZEALL);
    return true;
  default:
    break;
  }
