...
bool parse_dde_command(const std::string& cmd)
{
  ...
  for (size_t i = 0; i < cmd.size();)
  {
    if (cmd[i] == '[')
    {
      ...
      {
        // Send a message to open the file as if it were dropped into the
        // window.
        Event ev;
...
#pragma message("warning: Define PACKAGE macro with your app name")
    ...
    if (newApp && newTopic)
    {
      SendMessage(clienthwnd, WM_DDE_ACK, (WPARAM)hwnd,
  ...
  case WM_DDE_EXECUTE:
  {
    ...
    PostMessage(clienthwnd, WM_DDE_ACK, (WPARAM)hwnd,
                ReuseDDElParam(lparam, msg, WM_DDE_ACK, (UINT_PTR)ddeack.value,
                               (UINT_PTR)lparam));
