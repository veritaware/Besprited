...
public:
  ...
  [[nodiscard]] T x2() const { return x + w; }
  [[nodiscard]] T y2() const { return y + h; }
  ...
  [[nodiscard]] PointT<T> center() const
  {
  ...
  // rectangle).
  [[nodiscard]] PointT<T> origin() const { return PointT<T>(x, y); }

  // Returns point in the lower-right corner that is outside the
  // rectangle (x+w, y+h).
  [[nodiscard]] PointT<T> point2() const { return PointT<T>(x + w, y + h); }

  [[nodiscard]] SizeT<T> size() const { return SizeT<T>(w, h); }
  ...
  [[nodiscard]] bool contains(const PointT<T>& pt) const
  {
    return pt.x >= x && pt.x < x + w && pt.y >= y && pt.y < y + h;
  }

  [[nodiscard]] bool contains(const T& u, const T& v) const
  {
    return u >= x && u < x + w && v >= y && v < y + h;
  }

  // Returns true if this rectangle entirely contains the rc rectangle.
  [[nodiscard]] bool contains(const RectT& rc) const
  {
  ...
  [[nodiscard]] bool intersects(const RectT& rc) const
  {
    if (isEmpty() || rc.isEmpty())
      return false;
  ...
  [[nodiscard]] RectT createUnion(const RectT& rc) const
  {
    if (isEmpty())
      return rc;
    else if (rc.isEmpty())
  ...
  [[nodiscard]] RectT createIntersection(const RectT& rc) const
  {
    if (intersects(rc))
      return RectT(PointT<T>(x > rc.x ? x : rc.x, y > rc.y ? y : rc.y),
                   PointT<T>(x + w < rc.x + rc.w ? x + w : rc.x + rc.w,
