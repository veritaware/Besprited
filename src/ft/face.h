...
template <typename Cache> class FaceBase
{
public:
  FaceBase(FT_Face face)
    : m_face(face)
  {
  }
  ...
  [[nodiscard]] bool isValid() const { return (m_face != nullptr); }
  ...
  [[nodiscard]] double height() const
  {
    ...
    const double em_size = 1.0 * m_face->units_per_EM;
    const double y_scale = metrics->y_ppem / em_size;
    return static_cast<int>(m_face->height * y_scale) - 1;
  }

  [[nodiscard]] double ascender() const
  {
    FT_Size_Metrics* metrics = &m_face->size->metrics;
    const double em_size = 1.0 * m_face->units_per_EM;
    const double y_scale = metrics->y_ppem / em_size;
    return static_cast<int>(m_face->ascender * y_scale);
  }

  [[nodiscard]] double descender() const
  {
    FT_Size_Metrics* metrics = &m_face->size->metrics;
    const double em_size = 1.0 * m_face->units_per_EM;
    const double y_scale = metrics->y_ppem / em_size;
    return static_cast<int>(m_face->descender * y_scale);
  }
...
template <typename Cache> class FaceFT : public FaceBase<Cache>
{
public:
  ...
  void forEachGlyph(const std::string& str, Callback callback)
  {
    ...
    for (; it != end; ++it)
    {
      ...
      {
        FT_Vector kerning;
        FT_Get_Kerning(m_face, prev_glyph, glyph_index, FT_KERNING_DEFAULT,
                       &kerning);
        x += kerning.x / 64.0;
      }
      ...
      if (glyph)
      {
        ...
        const double width =
            glyph->ft_glyph->advance.x / static_cast<double>(1 << 16);
        glyph->x = x;
        glyph->y = y;
        ...
        glyph->width = static_cast<int>(
            std::max<double>(glyph->bearingX + width, glyph->bitmap->width));

        callback(*glyph);

        x += width;
        y += glyph->ft_glyph->advance.y / static_cast<double>(1 << 16);
  ...
  gfx::Rect calcTextBounds(const std::string& str)
  {
    ...
    forEachGlyph(str,
                 ...
                 {
                   bounds |=
                       gfx::Rect(static_cast<int>(glyph.x + glyph.bearingX),
                                 static_cast<int>(glyph.y + glyph.offsetY),
                                 static_cast<int>(glyph.ft_glyph->advance.x /
  ...
  Glyph* loadGlyph(FT_Face face, FT_UInt glyphIndex, bool antialias)
  {
    FT_Error err =
        FT_Load_Glyph(face, glyphIndex,
                      FT_LOAD_RENDER | (antialias ? FT_LOAD_TARGET_NORMAL
                                                  : FT_LOAD_TARGET_MONO));
    if (err)
      return nullptr;
...
public:
  ...
  Glyph* loadGlyph(FT_Face face, FT_UInt glyphIndex, bool antialias)
  {
    ...
    const Glyph* glyph = NoCache::loadGlyph(face, glyphIndex, antialias);
    if (!glyph)
      return nullptr;
