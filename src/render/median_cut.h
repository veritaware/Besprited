...
template <class Histogram> class Box
{
...
public:
  ...
  uint32_t meanColor(const Histogram& histogram) const
  {
    ...
    ASSERT(count > 0 && "Box without histogram points, you must fill the "
                        "histogram before using this function.");
    if (count == 0)
...
private:
                      ...
                      const int& l2) const
  {
    ...
    for (i = i1; i <= i2; ++i)
    {
      ...
      // As we move the plane to split through "i" axis One side is getting more
      // points,
