  ...
  : m_prereleaseDigit(0)
{
  ...
  while (!m_prerelease.empty() && m_prerelease[m_prerelease.size() - 1] == '.')
    m_prerelease.erase(m_prerelease.size() - 1);
}

bool Version::operator<(const Version& other) const
{
  auto it1 = m_digits.begin();
  const auto end1 = m_digits.end();
  auto it2 = other.m_digits.begin();
  const auto end2 = other.m_digits.end();

  while (it1 != end1 || it2 != end2)
  {
    const int digit1 = (it1 != end1 ? *it1++ : 0);
    const int digit2 = (it2 != end2 ? *it2++ : 0);
  ...
  {
    const int res = m_prerelease.compare(other.m_prerelease);
    if (res < 0)
      return true;
    else if (res > 0)
      return false;
