...
public:
  ...
  {
  public:
    Value(Option* option, std::string value)
      : m_option(option)
      , m_value(std::move(value))
    {
    }
    [[nodiscard]] const Option* option() const { return m_option; }
  ...
  [[nodiscard]] const OptionList& options() const { return m_options; }

  // List of specified options/values in the command line.
  [[nodiscard]] const ValueList& values() const { return m_values; }

  [[nodiscard]] bool enabled(const Option& option) const;
  [[nodiscard]] std::string value_of(const Option& option) const;
